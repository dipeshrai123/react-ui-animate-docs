...
function MyComponent() {
  ...
  return (
    <animate.div style={{ width, height: 100, background: '#7d93b0' }} />
  );
}
```
...
import Demo_modifier_withSpring_ArrayExample from '@site/src/examples/5.3.2/modifier/withSpring/ArrayExample';
  ...
  return (
    <>
      ...
      {heights.map((height, i) => (
        <animate.div
          key={i}
          style={{ height, background: '#7d93b0', margin: 5 }}
        />
      ))}
    </>
  );
