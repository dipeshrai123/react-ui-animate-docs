...
elements" pattern (tab indicators, expanding cards, shared images between a
...
`layoutOptions` sets the transition (spring or timing, options-only — no
...
function TabsB() {
  return (
    <LayoutGroup>
      {/* Same layoutId string, but scoped to this LayoutGroup — */}
      {/* won't transition with TabsA's "indicator". */}
