...
title: Interpolate
---
`interpolate` is a utility function that maps a numeric input value from one range (inRange) to another (outRange), supporting not only numbers but also strings with units, CSS functions, and even CSS color literals like rgb, hsl, and named colors.\
This is particularly useful for animations, transitions, and styling transformations.
